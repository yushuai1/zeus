...
import static com.api.mysqlUtil.mybitis.MybitisUtil.getSqlSession;
...
public class MybitisClientApi {
    ...
    public  MybitisClientApi(String ip,int port,String database,String name,String password,String dbType){
        session = getSqlSession(ip,port,database,name,password,dbType);
    ...
    public static void main(String[] args) {
        ...
        MybitisClientApi session = new MybitisClientApi(
                "localhost",3306,"test","root","root", UtilTool.MYSQL);
...
//        session.insert("INSERT into user values(#{name},#{id},#{age} )",user);
        ...
        for(int i=100;i<10100;i++){
            session.insert("INSERT INTO user_join (`name`, `age`, `id`, `classes`) VALUES ('asd', 11, 22, '54');");
//              t=session.selectList("select * from user_join",Test.class);
        }
        System.out.println((System.currentTimeMillis()-t1)+"-----"+t.get(0).toString());
