package com.序列化.protostuff;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.parser.ParserConfig;
import com.alibaba.fastjson.serializer.SerializeConfig;
import com.序列化.TestJson;
...
//        Map<String, Object> jk = (Map<String, Object>) deserializeWrapper.getData();
//        jk.forEach(
//                (k,v)->{
//                    System.out.println("反序列化map对象：" + k+"----"+v);
//                }
//        );
        ...
        SerializeDeserializeWrapper wrappers = SerializeDeserializeWrapper.builder(testJson);
        byte[] serializet = ProtostuffUtils.serialize(wrappers);
        System.out.println("序列化TestJson二进制数组长度 length=" + serializet.length);

        long t1 = System.currentTimeMillis();
        for (int i = 0; i < 10 * 10000; i++) {
            SerializeDeserializeWrapper deserialize = ProtostuffUtils.deserialize(serializet,
                    SerializeDeserializeWrapper.class);
            TestJson jks = (TestJson) deserialize.getData();
        }
        System.out.println(System.currentTimeMillis() - t1);

        String jsons = JSON.toJSONString(testJson);
        byte[] bbb = jsons.getBytes();
        System.out.println("json changdu"+jsons.getBytes().length);

        long t2 = System.currentTimeMillis();
        for (int i = 0; i < 10 * 10000; i++) {
            TestJson tj = com.alibaba.fastjson.JSON.parseObject(bbb, TestJson.class);
        }
